...
class LA(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        self.sample_list = []
        if split == 'train':
            self.image_list = glob(self._base_dir + '/Training Set/*/mri_norm2.h5')
        elif split == 'test':
            self.image_list = glob(self._base_dir + '/Testing Set/*/mri_norm2.h5')
        if num is not None:
            self.image_list = self.image_list[:num]
...
class ToTensorBra(object):
    """Convert ndarrays in sample to Tensors."""

    def __call__(self, sample):
        image = sample['image']
        image = image.reshape(1, image.shape[0], image.shape[1], image.shape[2]).astype(np.float32)
        return {'image': torch.from_numpy(image), 'label': torch.from_numpy(sample['label'].astype(np.uint8)).long()}
